...
\begin{itemize}
  \item Rebuilt the serial MATLAB workflow as a CUDA C++ parallel pipeline to address high-volume cross-correlation costs in 3D ultrasound speckle tracking
  \item Decomposed the algorithm into RF integral volume construction, cross-correlation integral volume generation, and 3D subpixel displacement estimation, with dedicated GPU kernels for each stage
...
\begin{itemize}[parsep=0.5ex]
  \item Programming and Parallel Computing: C/C++, Fortran, Python; CUDA, HIP, OpenMP, MPI, Sunway
  \item Performance Optimization: CPU/GPU architecture, CUDA operators, memory access optimization, communication optimization
\end{itemize}
